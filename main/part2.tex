...
La première consistait en une erreur de découpage dans les précis n° 48 bis et 66 ter. Le premier est scindé en dix fichiers contenant deux pages chacun (\citecode{s2t1\_chapt\_6.xml} à \citecode{s2t1\_chapt\_15.xml}) et le second en quatorze fichiers contenant quatre (\citecode{s2t3\_chapt\_14.xml}) et deux pages (\citecode{s2t3\_chapt\_15.xml} à \citecode{s2t3\_chapt\_27.xml}). Les deux pages sont toujours un recto et un verso.

Mis à part \citecode{s2t1\_chapt\_6.xml} et \citecode{s2t3\_chapt\_14} \citecode{.xml} qui commencent par le titre du précis, les \citecode{<body>} de ces fichiers ont un point commun : ils débutent tous par les deux mêmes lignes, \citecode{<p>\textsc{précis de monographie}</p> \textbackslash n <p>[numéro de la page]</p>}. Or ces deux informations --- le rappel du titre du chapitre et le numéro de la page courante --- se trouvent sur une seule et même ligne dans les images. Une erreur de fission horizontale résultant d'une mauvaise segmentation s'est donc produite au moment de l'\ocr\footcite[p. 5-6]{karpinski}.
...
Une question demeure néanmoins : pourquoi \citecode{s2t3\_chapt\_14.xml} contient-il quatre pages, à l'inverse des autres qui n'en contiennent que deux ? Le fait est que l'en-tête de la troisième page a été détecté convenablement, et donc retiré du flux du texte. 

L'erreur a été résolue par un transfert manuel du contenu de chaque fichier particulier dans un fichier global (\citecode{s2t1\_chapt\_6.xml} et \citecode{s2t3\_chapt\_14}). Le choix a été fait de ne pas modifier la numérotation des fichiers suivants, notamment afin de pouvoir effectuer un suivi sur la durée. C'est la raison pour laquelle, dans la deuxième série, le n°~6 est suivi du n°~16 dans le premier volume et le n°~14 par le n°~28 dans le troisième.

\subsection{Défaut de transcription}

La seconde erreur majeure à laquelle nous avons été confronté est le défaut d'une partie de la transcription dans un chapitre (\ann{} \ref{ann:deficit-transcr}). Il ne s'agit pas d'une erreur de découpage, puisque déjà présente dans les treize fichiers source. C'est néanmoins l'opération de contrôle du découpage qui a permis de s'en rendre compte.

\texttt{<figure>} et \citecode{<figure>}
